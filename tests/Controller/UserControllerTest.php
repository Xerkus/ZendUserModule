...
namespace Zend\UserModuleTest\Controller;

use Zend\Form\FormElementManager;
use Zend\UserModule\Controller\RedirectCallback;
use Zend\UserModule\Controller\UserController as Controller;
use Zend\Http\Response;
use Zend\Stdlib\Parameters;
use Zend\ServiceManager\ServiceLocatorInterface;
use Zend\UserModule\Service\User as UserService;
use Zend\Form\Form;
use Zend\UserModule\Options\ModuleOptions;
use Zend\UserModule\Entity\User as UserIdentity;
...
class UserControllerTest extends \PHPUnit_Framework_TestCase
{
    ...
    protected $zendUserAuthenticationPlugin;
    ...
    {
        $this->redirectCallback = $this->getMockBuilder('Zend\UserModule\Controller\RedirectCallback')
        ...
        $this->zendUserAuthenticationPlugin = $this->getMock('Zend\UserModule\Controller\Plugin\UserAuthentication');
    ...
    public function setUpZendUserAuthenticationPlugin($option)
    {
        if (array_key_exists('hasIdentity', $option)) {
            ...
            $this->zendUserAuthenticationPlugin->expects($this->any())
                ->method('hasIdentity')
                ->will($return);
        }
        ...
        $this->pluginManagerPlugins['zendUserAuthentication'] = $this->zendUserAuthenticationPlugin;

        return $this->zendUserAuthenticationPlugin;
    }
    ...
    public function testActionControllHasIdentity($methodeName, $hasIdentity, $redirectRoute, $optionGetter)
    {
        ...
        $this->setUpZendUserAuthenticationPlugin(array(
    ...
    public function testLoginActionValidFormRedirectFalse($isValid, $wantRedirect)
    {
        ...
        $plugin = $this->setUpZendUserAuthenticationPlugin(array(
            'hasIdentity'=>false
        ));
        ...
        $form = $this->getMockBuilder('Zend\UserModule\Form\Login')
            ->disableOriginalConstructor()
            ->getMock();
        ...
        if ($isValid) {
            $adapter = $this->getMock('Zend\UserModule\Authentication\Adapter\AdapterChain');
    ...
    public function testLoginActionIsNotPost($redirect)
    {
        ...
        $this->assertInstanceOf('Zend\UserModule\Form\Login', $result['loginForm']);
    ...
    public function testRegisterAction($wantRedirect, $postRedirectGetReturn, $registerSuccess, $loginAfterSuccessWith)
    {
        ...
        $userService = $this->getMock('Zend\UserModule\Service\User');
        ...
        if ($registerSuccess) {
            ...
            $user->setEmail('zend-user@trash-mail.com');
            $user->setUsername('zend-user');
    ...
    public function providerTestSetterGetterServices()
    {
        ...
        return array(
            ...
            array('UserService', true, new UserService(), 'zenduser_user_service' ),
            array('UserService', false, new UserService(), null ),
            array('RegisterForm', true, new Form(), 'zenduser_register_form' ),
            array('RegisterForm', false, new Form(), null ),
            array('ChangePasswordForm', true, new Form(), 'zenduser_change_password_form' ),
            array('ChangePasswordForm', false, new Form(), null ),
            array('ChangeEmailForm', true, new Form(), 'zenduser_change_email_form' ),
            array('ChangeEmailForm', false, new Form(), null ),
            array('LoginForm', true, new Form(), 'zenduser_login_form', $loginFormCallback[0] ),
            array('LoginForm', true, new Form(), 'zenduser_login_form', $loginFormCallback[1] ),
            array('LoginForm', false, new Form(), null, $loginFormCallback[0] ),
            array('LoginForm', false, new Form(), null, $loginFormCallback[1] ),
            array('Options', true, new ModuleOptions(), 'zenduser_module_options' ),
    ...
    public function providerTestRegisterAction()
    {
        $registerPost = array (
            'username'=>'zend-user',
            'email'=>'zend-user@trash-mail.com',
            'password'=>'secret'
        );
