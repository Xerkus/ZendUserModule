...
use Zend\UserModule\Mapper\User as Mapper;
use Zend\UserModule\Entity\User as Entity;
use Zend\Db\ResultSet\HydratingResultSet;
use Zend\Db\Adapter\Adapter;
use Zend\UserModule\Mapper\UserHydrator;

class UserTest extends \PHPUnit_Framework_TestCase
{
    /** @var \Zend\UserModule\Mapper\User */
    protected $mapper;
...
//         $this->setUpAdapter('pgsql');
    ...
    public function testIntegrationFindBy($methode, $args, $expectedParams, $eventListener, $entityEqual)
    {
        ...
        foreach ($this->realAdapter as $dbAdapter) {
            ...
            $this->assertInstanceOf('Zend\UserModule\Entity\User', $return);
            $this->assertEquals($entityEqual, $return);
        }
    ...
    {
        $this->mapper->setTableName('ZendUser');
        $this->assertEquals('ZendUser', $this->mapper->getTableName());
    }

    public function testInsertUpdateDelete()
    {
        $baseEntity = new Entity();
        $baseEntity->setEmail('zend-user-foo@zend-framework.org');
        $baseEntity->setUsername('zend-user-foo');
        $baseEntity->setPassword('zend-user-foo');
    ...
    {
        $user = new Entity();
        $user->setEmail('zend-user@github.com');
        $user->setUsername('zend-user');
        $user->setDisplayName('Zfc-User');
        $user->setId('1');
        $user->setState(1);
        $user->setPassword('zend-user');
