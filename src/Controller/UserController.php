...
namespace Zend\UserModule\Controller;
...
use Zend\UserModule\Options\UserControllerOptionsInterface;

class UserController extends AbstractActionController
{
    const ROUTE_CHANGEPASSWD = 'zenduser/changepassword';
    const ROUTE_LOGIN        = 'zenduser/login';
    const ROUTE_REGISTER     = 'zenduser/register';
    const ROUTE_CHANGEEMAIL  = 'zenduser/changeemail';

    const CONTROLLER_NAME    = 'zenduser';
    ...
    {
        if (!$this->zendUserAuthentication()->hasIdentity()) {
    ...
    public function loginAction()
    {
        ...
        // clear adapters
        $this->zendUserAuthentication()->getAuthAdapter()->resetAdapters();
        $this->zendUserAuthentication()->getAuthService()->clearIdentity();
    ...
    {
        $this->zendUserAuthentication()->getAuthAdapter()->resetAdapters();
        $this->zendUserAuthentication()->getAuthAdapter()->logoutAdapters();
        $this->zendUserAuthentication()->getAuthService()->clearIdentity();
    ...
    public function authenticateAction()
    {
        ...
        $adapter = $this->zendUserAuthentication()->getAuthAdapter();
    ...
    {
        if (!$this->userService) {
            $this->userService = $this->serviceLocator->get('zenduser_user_service');
        }
    ...
    public function getRegisterForm()
    {
        if (!$this->registerForm) {
            $this->setRegisterForm($this->serviceLocator->get('zenduser_register_form'));
        }
    ...
    {
        if (!$this->loginForm) {
            $this->setLoginForm($this->serviceLocator->get('zenduser_login_form'));
        }
    ...
    public function getChangePasswordForm()
    {
        if (!$this->changePasswordForm) {
            $this->setChangePasswordForm($this->serviceLocator->get('zenduser_change_password_form'));
        }
    ...
    public function getOptions()
    {
        if (!$this->options instanceof UserControllerOptionsInterface) {
            $this->setOptions($this->serviceLocator->get('zenduser_module_options'));
        }
        return $this->options;
    }
    ...
    public function getChangeEmailForm()
    {
        if (!$this->changeEmailForm) {
            $this->setChangeEmailForm($this->serviceLocator->get('zenduser_change_email_form'));
        }
